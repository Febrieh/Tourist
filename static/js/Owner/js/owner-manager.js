...
async function fetchReviewsData(resortId) {
    try {
        ...
        if (snapshot.exists()) {
            ...
            // Initialize sentiment tracking by year (2019-2024)
            const sentimentByYear = {};
            for (let year = 2019; year <= 2024; year++) {
                sentimentByYear[year] = { positive: 0, neutral: 0, negative: 0 };
            }
            ...
            for (let key in reviewsData) {
                if (key.startsWith('review_')) {
                    ...
                    // Ensure the timestamp is valid and extract the correct year
                    const timestamp = review.time ? new Date(review.time * 1000) : null;
                    const reviewYear = timestamp ? timestamp.getFullYear() : null;

                    // Check if the year is within the expected range
                    if (reviewYear && reviewYear >= 2019 && reviewYear <= 2024) {
                        if (rating >= 4) sentimentByYear[reviewYear].positive++;
            ...
            const sentimentData = Object.keys(sentimentByYear).map(year => ({
                year: parseInt(year), 
            ...
            // Render the sentiment trend chart and top keywords chart
